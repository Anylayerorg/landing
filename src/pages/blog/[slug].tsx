...
const BlogPost = () => {
  ...
  const portableTextComponents = {
    block: {
      h2: ({ children }: any) => <h2 className="text-3xl font-bold text-white mt-12 mb-6 tracking-tight leading-tight">{children}</h2>,
      h3: ({ children }: any) => <h3 className="text-2xl font-bold text-white mt-10 mb-5 tracking-tight">{children}</h3>,
      h4: ({ children }: any) => <h4 className="text-xl font-bold text-white/90 mt-8 mb-4 tracking-tight">{children}</h4>,
            ...
            <div className="max-w-4xl mx-auto space-y-10">
              ...
              >
                ...
                <div className="flex items-center gap-3 text-sm">
                  <span className="px-3 py-1 bg-white/[0.08] text-white border border-white/10 rounded-full font-bold uppercase tracking-[0.15em] text-[10px]">
                    {post.category}
                  </span>
                </div>
            ...
            <section className="px-6 md:px-12 mt-20 mb-16">
              <div className="max-w-4xl mx-auto">
                <div className="space-y-8">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-white/30">
                    Continue Reading
                  </h3>
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    {post.related.map((related: any, index: number) => (
                      <Link key={index} href={`/blog/${related.slug}`}>
                        ...
                        >
                          <h4 className="text-lg font-bold tracking-tight leading-tight group-hover:text-white/90 transition-colors mb-4">
                            {related.title}
                          </h4>
